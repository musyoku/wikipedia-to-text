...
def makeInternalLink(title, label):
    ...
    if Extractor.keepLinks:
        return '<a href="%s">%s</a>' % (urllib.quote(title),
                                        label)
    else:
        return label
...
def makeExternalLink(url, anchor):
    ...
    if Extractor.keepLinks:
        return '<a href="%s">%s</a>' % (urllib.quote(url),
                                        anchor)
    else:
        return anchor
        ...
        # handle lists
            ...
            if line:  # FIXME: n is '"'
                if Extractor.keepLists:
                    ...
                    bullet = '1. ' if n == '#' else '- '
                    page.append(
                        '{0:{1}s}'.format(bullet, len(listLevel)) + line)
...
def load_templates(file, output_file=None):
    ...
    for page_count, page_data in enumerate(pages_from(file)):
        ...
        if not output_file and (not templateNamespace
                                or not moduleNamespace):  # do not know it yet
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    if Extractor.expand_templates:
        ...
        logging.info("Loaded %d templates in %.1fs", len(templates),
    ...
    reduce = Process(
        ...
        args=(output_queue, spool_length, out_file, file_size, file_compress))
    reduce.start()
    ...
    for i in range(worker_count):
        extractor = Process(
            target=extract_process, args=(i, jobs_queue, output_queue))
    ...
    for page_data in pages_from(input):
        ...
        if ns not in templateKeys:
            ...
            try:
                e = Extractor(id, title, page)  # (id, title, page)
